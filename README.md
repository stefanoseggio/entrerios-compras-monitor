...
- **No true `UPDATED` event — and a sharper consequence than "looks like a new listing".** `record_id` deliberately excludes `estado` so a status change keeps the same id, but `record_id` is a hash of the 4 free-text fields (`procedimiento` + `objeto` + `destino` + `organismo`), and the source issues no native row id to correlate old and new text against. If the source silently corrects any of those 4 fields — a typo fix, a re-punctuated `objeto`, a renamed `organismo` — the hash changes, so the OLD `record_id` disappears from this run's fetch entirely. On the next **unfiltered** run that record doesn't just "look like a new listing": it is reported as `CLOSED` (the old id, wrongly, since the underlying procurement never actually closed) *and* `NEW_LISTING` (the new id) in the same run, and the old id is permanently deleted from delta state. There is no way to distinguish this from a real closure-plus-unrelated-new-listing pair from the outside — treat a `CLOSED`/`NEW_LISTING` pair with matching `organismo` and near-identical `objeto`/`destino` text in the same run as a plausible instance of this, not necessarily two independent events.
