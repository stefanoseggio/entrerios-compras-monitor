 ...
 */
export function applyDateRangeFilter<T>(records: readonly T[], dateRange: DateRangeCode | undefined): T[] {
    if (!dateRange) return [...records];

    log.warning(
        `dateRange="${dateRange}" was set, but this source publishes no per-record date field to filter on ` +
            '(verified live - see AGENTS.md). No date-based filtering was applied; every record is returned as usual.',
    );
    return [...records];
}
