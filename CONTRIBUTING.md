...
This repository ships the real, buildable TypeScript source for the **Entre Rios Argentina Licitaciones - Tender Delta API** Apify Actor (`entrerios-compras-monitor`). It is independently maintained by Stefano Seggio as part of the [Delta Registry](https://github.com/stefanoseggio) fleet — there is no separate contributor team, but external bug reports, source-coverage proposals, and documentation fixes are welcome.
...
No third-party credentials are required — the Entre Rios tender register is a public provincial government listing with no login wall or provider key of any kind.
...
Local runs hit the real, live Entre Rios UCC tender listing — there is no bundled fixture/mock server. The source has no native pagination, so every run (local or scheduled) fetches the entire live backlog (over 5,000 rows) in a single request regardless of `maxItems`; use a narrow filter (e.g. a specific `organismo`) while developing to keep the output small, since request volume itself can't be reduced from the input side.
...
For questions that aren't a code change (pricing, licensing, enterprise inquiries), use the Apify Store's Issues tab on the [live Actor page](https://apify.com/stefano_seggio/entrerios-compras-monitor) rather than a GitHub issue.
